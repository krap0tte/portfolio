...
0. **Inventaire** : les sous-dossiers de `photos/` sont relevés ; tout dossier de premier niveau hors des deux axes est une erreur (typiquement un reliquat de l'ancienne arborescence à un seul niveau).
1. **Sources non-webp** (JPG/PNG/TIFF déposés) → converties en WebP (`cwebp -q 82 -m 6`), **EXIF retiré** (`-metadata none`), **résolution d'origine conservée** ; **la source est ensuite supprimée** (le dépôt ne garde que le WebP). Le hero de `config.toml` suit la même règle.
2. **Appariement**, vérifié dans les deux sens : chaque dossier `<axe>/<slug>/` doit avoir son `content/<axe>/<slug>.md` **et réciproquement** ; chacun (catégorie comme série) doit y déclarer un `cover` existant, les séries en plus un `hero` existant ; `config.toml` un `hero` existant sous `static/`. Échoue sinon avec la liste de ce qui est mal apparié (`check_photo_field()` factorise le contrôle des champs pointant une photo).
...
4. **`data/photos.toml`** est régénéré intégralement depuis les sous-dossiers présents (les deux axes confondus dans `all`, puis un bloc `[[categories]]` / `[[series]]` par dossier), et le script indique s'il était **déjà à jour** ou s'il a été **mis à jour**.
